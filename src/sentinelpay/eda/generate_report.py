...
def render_phase_h_report(results: dict, report_path: Path) -> None:
    holdout_ladder = results["ladder_results_holdout"]
    grad = results["graduation_holdout"]
    g_ref = results["phase_g_validation_reference"]

    lines: list[str] = []
    lines.append("# SentinelPay -- Phase H Final Sealed-Holdout Evaluation Report")
    lines.append("")
    lines.append(
        "**Generated deterministically by `sentinelpay.eda.generate_report.render_phase_h_report` "
        "from `reports/eda/phase_h_results.json`** -- every number below is read from that file. "
        "This report was produced by the ONE authorized run of `python -m sentinelpay.eda.run_phase_h`; "
        "it cannot be regenerated by re-running that script (the one-shot guard refuses)."
    )

    lines.append(_PHASE_H_SCOPE_SECTION)

    lines.append("## 1. Row accounting\n")
    lines.append(f"- Total rows loaded (ALL FIVE partitions, including `holdout` for the first and only time): **{results['n_rows_total']:,}**.\n")
    lines.append("| partition | n_rows |")
    lines.append("|---|---|")
    for name, count in results["partition_counts"].items():
        lines.append(f"| {name} | {count:,} |")
    lines.append(
        f"\n- `payment_proxy_key` present on **{results['n_rows_valid_key']:,}** rows "
        f"(**{results['n_rows_missing_payment_proxy_key']:,}** excluded).\n"
        f"- Model rows: `train`=**{results['n_rows_train']:,}** (fit), `holdout`=**{results['n_rows_holdout']:,}** "
        f"(the one sealed evaluation)."
    )

    lines.append("\n## 2. Fixed ladder feature schema evaluated on holdout (B2/F1/F2 only)\n")
    for step in ["B2", "F1", "F2"]:
        cols = results["ladder_feature_columns"][step]
        lines.append(f"- **{step}** ({len(cols)} features): `{cols}`\n")

    lines.append("\n## 3. HOLDOUT results (Phase H -- the one sealed evaluation)\n")
    holdout_rows = [
        {"step": step, "n_features": r["n_features"], "converged": r.get("converged", ""), "roc_auc": r["roc_auc"], "pr_auc": r["pr_auc"]}
        for step, r in holdout_ladder.items()
    ]
    lines.append(_table(holdout_rows, float_fmt="{:.6f}"))

    lines.append(
        "\n## 4. VALIDATION reference (Phase G -- NOT re-evaluated here, quoted verbatim for comparison only)\n"
    )
    g_rows = [
        {"step": step, "n_features": r["n_features"], "roc_auc": r["roc_auc"], "pr_auc": r["pr_auc"]}
        for step, r in g_ref["ladder_results"].items()
    ]
    lines.append(_table(g_rows, float_fmt="{:.6f}"))
    lines.append(
        f"\nPhase G's own graduation verdict (validation): **all_gates_pass = {g_ref['graduation']['all_gates_pass']}**\n"
    )
    ...
    lines.append(
        ...
        f"- **Gate 4** -- PR-AUC(F1) > PR-AUC(B2), deterministic: **{grad['gate4_pr_auc_f1_gt_b2']}**\n"
        f"- Reported (not a gate): B2 <= F1 <= F2 monotonic in PR-AUC: **{grad['monotonic_b2_le_f1_le_f2_pr_auc']}**\n\n"
        f"### ALL HOLDOUT GATES PASS: **{grad['all_gates_pass']}**\n"
    )

    lines.append(
        "\n## 6. Finality\n\n"
        "This is the one sealed holdout evaluation for this frozen design. No further tuning, re-running, "
        "or threshold adjustment follows from this report -- any code change to `target_history.py`/"
        "`model_features.py`/`model_evaluation.py` after this report permanently invalidates it as evidence "
        "for this design.\n"
    )
